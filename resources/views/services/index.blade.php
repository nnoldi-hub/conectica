            ...
            @forelse ($services as $service)
                <article class="flex min-w-0 flex-col rounded-3xl border border-white/10 bg-white/[0.04] p-8 transition hover:border-cyan-400/40">
                    ...
                    <h2 class="mt-6 text-2xl font-semibold text-white">
                        <a href="{{ route('services.show', $service) }}" data-track-event="cta_click" data-track-target="services_list_title_{{ $service->slug }}" class="transition hover:text-cyan-700">{{ $service->title }}</a>
                    </h2>
                    ...
                    <div class="mt-auto flex flex-col gap-4 pt-8">
                        ...
                        @endif
                        <a href="{{ route('services.show', $service) }}" data-track-event="cta_click" data-track-target="services_list_details_{{ $service->slug }}" class="inline-flex items-center justify-center rounded-full border border-cyan-400/40 px-5 py-2.5 text-sm font-semibold text-cyan-300 transition hover:border-cyan-300 hover:bg-cyan-400/10 hover:text-cyan-200">
                            Vezi detaliile
                        </a>
                    </div>
                </article>
